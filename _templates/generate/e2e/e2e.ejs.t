...
import { <%= h.inflection.undasherize(name) %>Entity } from '@apis/<%= name %>/entities/<%= name %>.entity';
...
describe('<%= h.inflection.undasherize(name) %>Resolver (e2e)', () => {
	...
	let <%= h.inflection.camelize(name, true) %>: <%= h.inflection.undasherize(name) %>Entity;
