...
import { UseBookInterceptor, UseBookPaginatedInterceptor } from './book.interceptor';
...
export class BookController {
	...
	@Get()
	@ApiGetAll(BookResponse, 'Book')
	@UseBookPaginatedInterceptor()
	getAll(@Query() query: PaginationDto) {
		return this.commandBus.execute(new GetAllBookPaginatedCommand({ query }));
	}

	@Get(':id')
	@ApiGetOne(BookResponse, 'Book')
